# 네트워크 기반 - 질량 확산 접근법
# 상대적인 값이라서, 추천된 결과값의 최대, 최소 값을 보고 판단하는 게 좋을 것 같다. (threshold 를 50으로 잡고 추천-3 이상, 비추천- 2 이하)
# 또는 그냥 threshold를 절대값으로 할 지 고민해보기

_INPUT_NAME = 'C:\hanyang\paper\data_3_train.csv'
_OUTPUT_NAME = 'C:\hanyang\paper\output\data_3_train_result_network1.csv'
        ...
        for m_item_idx in row_con[j]:
            for c_user_idx in item_con[m_item_idx]:
                if c_user_idx not in user_val:
                    user_val[c_user_idx] = item_val[m_item_idx] / len(item_con[m_item_idx])
                else:
                    user_val[c_user_idx] += item_val[m_item_idx] / len(item_con[m_item_idx])
        ...
        for uid in user_val:
            for item_id in row_con[uid]:
                ...
                if item_id not in item_val:
                    item_val[item_id] = user_val[uid] / len(row_con[uid])
                else:
                    item_val[item_id] += user_val[uid] / len(row_con[uid])
